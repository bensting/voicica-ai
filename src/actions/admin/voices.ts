    ...
    for (const voice of localeVoices) {
      ...
      await prisma.voices.create({
        data: {
          ...
          voice_sample_text: '',
          tags: [],
          style_list: voice.StyleList && voice.StyleList.length > 0 ? voice.StyleList : ['default'],
    ...
    for (const dbVoice of dbVoices) {
      ...
      await prisma.voices.update({
        ...
        data: {
          ...
          style_list: azureVoice.StyleList && azureVoice.StyleList.length > 0 ? azureVoice.StyleList : ['default'],
