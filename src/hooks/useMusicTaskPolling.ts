...
import { getMusicTaskStatus, type MusicRecord } from '@/actions/music';
...
interface UseMusicTaskPollingOptions {
  /** 音乐记录列表 */
  records: MusicRecord[];
  /** 是否启用轮询 */
  enabled?: boolean;
  ...
  interval?: number;
  /** 状态更新回调 */
  onStatusUpdate: (taskId: string, status: Awaited<ReturnType<typeof getMusicTaskStatus>>) => void;
}
...
export function useMusicTaskPolling({
  records,
  enabled = true,
  interval = 5000,
  onStatusUpdate,
}: UseMusicTaskPollingOptions) {
  ...
  const pollTasks = useCallback(async () => {
    ...
    const processingRecords = records.filter(
      (m) =>
        ...
        isWithinTimeout(m.created_at)
    );

    if (processingRecords.length === 0) return;

    console.log(`🎵 [useMusicTaskPolling] 轮询 ${processingRecords.length} 个处理中的任务`);
    ...
    await Promise.all(
      processingRecords.map(async (record) => {
        try {
          ...
          console.error(`🎵 [useMusicTaskPolling] 查询任务状态失败: ${record.task_id}`, error);
        }
      })
    );
