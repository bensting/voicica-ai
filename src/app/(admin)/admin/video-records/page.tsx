...
import {
  getVideoRecords,
  getVideoRecordsStats,
  deleteVideoRecord,
  deleteVideoRecords,
  VideoRecordItem,
} from '@/actions/admin/video-records';
...
export default function VideoRecordsPage() {
  const [records, setRecords] = useState<VideoRecordItem[]>([]);
  ...
  // 详情弹窗
  const [detailRecord, setDetailRecord] = useState<VideoRecordItem | null>(null);
  ...
  const handleBatchDelete = async () => {
    ...
    const result = await deleteVideoRecords(selectedIds);
    if (result.success) {
      setSelectedIds([]);
      loadData();
    } else {
  ...
  return (
    <div>
      {/* 页面标题 */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Video 记录管理</h1>
        <p className="text-gray-600 mt-1">查看和管理 AI 视频生成记录</p>
      </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div>
            ...
            >
              ...
              <option value="kling-v2-pro">Kling v2 Pro</option>
              <option value="kling-v1.5-pro">Kling v1.5 Pro</option>
              <option value="hailuo-t2v-01">Hailuo T2V</option>
            </select>
          </div>
          <div>
              ...
              <tbody className="divide-y divide-gray-200">
                {records.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      {record.actualDuration ? `${record.actualDuration}s` : `${record.duration}s`}
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col mx-4">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
              <h3 className="text-lg font-semibold text-gray-900">Video 记录详情</h3>
              <button
                onClick={() => setDetailRecord(null)}
            ...
            <div className="flex-1 overflow-auto p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-sm text-gray-900">{detailRecord.resolution}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-500">比例</div>
                  <div className="text-sm text-gray-900">{detailRecord.aspectRatio}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-500">目标时长</div>
                  <div className="text-sm text-gray-900">{detailRecord.duration}s</div>
                </div>
                <div>
                  <div className="text-sm text-gray-500">实际时长</div>
                  <div className="text-sm text-gray-900">{detailRecord.actualDuration ? `${detailRecord.actualDuration}s` : '-'}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-500">消耗积分</div>
              ...
              {detailRecord.videoUrl && (
                <div>
                  <div className="text-sm text-gray-500 mb-2">视频预览</div>
                  <video controls className="w-full max-h-64 rounded-lg bg-black">
                    <source src={detailRecord.videoUrl} type="video/mp4" />
                    您的浏览器不支持视频播放
                  </video>
                </div>
              )}
