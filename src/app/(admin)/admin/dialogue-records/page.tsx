 ...
 * Dialogue 记录管理页面
 */
export default function DialogueRecordsPage() {
  const [records, setRecords] = useState<DialogueRecordItem[]>([]);
  ...
  // 详情弹窗
  const [detailRecord, setDetailRecord] = useState<DialogueRecordItem | null>(null);
  ...
  const handleDelete = async (id: number) => {
    ...
    const result = await deleteDialogueRecord(id);
    if (result.success) {
      loadData();
    } else {
  ...
  const handleBatchDelete = async () => {
    ...
    const result = await deleteDialogueRecords(selectedIds);
    if (result.success) {
      setSelectedIds([]);
      loadData();
    } else {
  ...
  // 解析对话 JSON
  const parseDialogueJson = (json: string): DialogueSegment[] => {
    try {
  ...
  // 获取对话预览
  const getDialoguePreview = (json: string, maxLength: number = 50): string => {
    const segments = parseDialogueJson(json);
  ...
  return (
    <div>
      {/* 页面标题 */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Dialogue 记录管理</h1>
        <p className="text-gray-600 mt-1">查看和管理 AI 对话生成记录</p>
      </div>
                  ...
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div
                        ...
                        title={getDialoguePreview(record.dialogueJson, 200)}
                        onClick={() => setDetailRecord(record)}
                      >
                        {getDialoguePreview(record.dialogueJson)}
                      </div>
                    </td>
                    <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col mx-4">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
              <h3 className="text-lg font-semibold text-gray-900">Dialogue 记录详情</h3>
              <button
                onClick={() => setDetailRecord(null)}
            ...
            <div className="flex-1 overflow-auto p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-sm text-gray-500">总字符数</div>
                  <div className="text-sm text-gray-900">{detailRecord.totalCharacters.toLocaleString()}</div>
                </div>
              </div>

              <div>
                <div className="text-sm text-gray-500 mb-2">对话内容</div>
                <div className="bg-gray-50 rounded-lg p-3 space-y-2 max-h-60 overflow-auto">
                  {parseDialogueJson(detailRecord.dialogueJson).map((segment, index) => (
