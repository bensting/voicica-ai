...
interface DialogueDetailModalProps {
  ...
  onClose: () => void;
  onRecreate: () => void;
  onDelete: (dialogue: DialogueRecord) => void;
}
...
export default function DialogueDetailModal({
  dialogue,
  onClose,
  onRecreate,
  onDelete,
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[#0a0a1a] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-4">
        ...
        {/* 标题 */}
        <h1 className="text-2xl font-bold text-white text-center mb-4">AI Dialogue</h1>
      ...
      <div className="flex-shrink-0 px-6 pb-8 bg-[#0a0a1a]">
        ...
        >
          <div
            className="h-full bg-gradient-to-r from-indigo-400 to-pink-500 rounded-full relative"
            style={{ width: `${progress}%` }}
          >
            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-pink-400 rounded-full" />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <p className="text-gray-500 text-sm flex-1 truncate">
            {dialogue.total_characters} characters
          </p>
          <button
